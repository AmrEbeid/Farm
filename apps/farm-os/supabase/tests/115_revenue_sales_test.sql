...
-- 20260701460000: the budget.write gate, the delivery-before-price mechanic (pending sale posts NOTHING and
-- keeps total NULL — never a fabricated 0, #1), fn_finalize_sale_price posting Dr ذمم مدينة / Cr إيرادات,
-- collections clearing A-R + deriving payment_status, the Σ(collections) ≤ total guard, cross-org guards,
...
select is(
  ...
  'partially_collected', 'payment_status derives to partially_collected after 2000 of 5000');
select is(
  (select jl.debit from public.journal_lines jl join public.journal_entries je on je.id = jl.journal_entry_id
     join public.accounts a on a.id = jl.account_id
...
select is(
  (select payment_status from public.sales where id = current_setting('test.sale')::uuid),
  'collected', 'payment_status derives to collected once Σ(collections) = total');
