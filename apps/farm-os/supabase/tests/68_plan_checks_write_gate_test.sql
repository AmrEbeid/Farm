...
-- mask-by-DELETE is blocked too (RESTRICTIVE for-delete policy): the member's DELETE is filtered to 0
-- rows, so the seeded check SURVIVES (a filtered delete is silent, not 42501 — assert the row remains).
select lives_ok(
