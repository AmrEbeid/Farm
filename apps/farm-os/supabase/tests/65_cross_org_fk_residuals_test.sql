...
-- 4b) ...AND via a direct PARTITION-CHILD insert (the bypass the #309 review caught: authenticated
-- holds direct DML on farm_event_2025_07, so the per-partition policy must carry the same check)
select throws_ok(
