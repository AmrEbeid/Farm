  ...
  (select count(*)::int
    ...
    where n.nspname = 'public'
      ...
      and (p.proname not in (
        ...
        'fn_cost_center_reports_snapshot',         -- exact atomic cost-center report snapshot (migration 20260822141900)
        'fn_payment_request_detail_snapshot',      -- exact atomic payment-request 360 snapshot (migration 20260822142000)
        ...
        'fn_custody_dashboard_summary',            -- atomic exact custody dashboard read (migration 20260822140700)
        ...
        'fn_revenue_sales_report_exact',       -- exact-transport revenue/A-R report wrapper (migration 20260822141000)
        'fn_pending_sale_pricing', 'fn_open_sale_receivables', -- bounded exact daily receivables reads (migration 20260822140900)
        ...
        'fn_cost_center_history_summary', -- read-only annual cost-center aggregate (migration 20260822140400)
        'fn_cost_center_revenue_summary', -- exact posted-sale revenue by cost center (migration 20260822140500)
        'fn_expense_register_summary', -- read-only exact expense-register summary (migration 20260730140000)
        'fn_month_close_summary', -- read-only exact close-checklist summary (migration 20260822140300)
        'fn_set_missing_expense_date', -- one-time undated-expense correction (migration 20260822140300)
        ...
        'fn_save_marketing_contact_v3', -- atomic contact-status update preserving metadata (SPEC-0032, migration 20260822142200)
        ...
        ,'fn_save_marketing_workspace_control' -- exact source-layout draft values (SPEC-0032, migration 20260822142300)
        ,'fn_marketing_workspace_aggregates' -- all-row workspace insight aggregates (SPEC-0032, migration 20260822142400)
