-- Farm OS - plan-operation positive requirement backstop.
--
-- F2 already fixed the OperationBuilder/server-action path so blank rows are dropped and typed
-- zero/negative values are rejected before persistence. This migration tightens the lower layers so
-- future direct RPC/table writers cannot reintroduce fabricated plan requirements.
--
-- DB-layer guard:
--   * material qty is already NOT NULL (0099) and positive (20260701410000).
--   * labor count/days were positive CHECKs, but CHECK (count > 0) still admits NULL. Rebuild them
--     with explicit IS NOT NULL while keeping NOT VALID so historical rows do not block deploy.
--
-- RPC guard:
--   * re-emit the latest frozen 16-arg fn_add_plan_operation_multi body from
--     20260706094500_plan_operation_soil_reading_guard.sql.
--   * reject material qty and labor count/days when missing/NULL or <= 0 with clean 22023 errors,
--     before any requirement row is written.

alter table public.plan_labor_requirements
  drop constraint if exists plan_labor_requirements_count_positive;
alter table public.plan_labor_requirements
  add constraint plan_labor_requirements_count_positive
  check (count is not null and count > 0) not valid;

alter table public.plan_labor_requirements
  drop constraint if exists plan_labor_requirements_days_positive;
alter table public.plan_labor_requirements
  add constraint plan_labor_requirements_days_positive
  check (days is not null and days > 0) not valid;
...
begin
  ...
  -- materials: each item must be in the plan's org; qty is required and positive; optional compliance
  -- fields (from PR #562/Layer 2) retain their target_zone/applicator validation and nullif unit path.
  for v_mat in select * from jsonb_array_elements(coalesce(p_materials, '[]'::jsonb)) loop
    ...
    v_mat_qty := (v_mat->>'qty')::numeric;
    if v_mat_qty is null or v_mat_qty <= 0 then
      raise exception 'material qty must be positive' using errcode = '22023';
    end if;
    ...
    values (
      v_org, v_op_id, (v_mat->>'item_id')::uuid, v_mat_qty, nullif(v_mat->>'unit', ''),
  ...
  -- labour: count/days are required and positive; an OPTIONAL person_id (from PR #549/Layer 1) must
  -- be an ACTIVE same-org person - mirrors the assignee validation below.
  for v_lab in select * from jsonb_array_elements(coalesce(p_labor, '[]'::jsonb)) loop
    v_lab_count := (v_lab->>'count')::int;
    v_lab_days := (v_lab->>'days')::numeric;
    if v_lab_count is null or v_lab_count <= 0 or v_lab_days is null or v_lab_days <= 0 then
      raise exception 'labour count/days must be positive' using errcode = '22023';
    end if;
    ...
    values (v_org, v_op_id, v_lab->>'person_or_team', v_lab_count, v_lab_days, v_lab_person);
    v_n_lab := v_n_lab + 1;
  end loop;
