-- Farm OS — engine display fix: align the coverage WARNING message with the max-deficit sizing basis.
--
-- DISPLAY-ONLY. recommend_qty is UNCHANGED. Since migration 0055 (#280 F4), fn_stock_coverage sizes
-- recommend_qty off v_maxdef = the DEEPEST projected deficit across the whole horizon
-- (v_raw := greatest(0, v_maxdef + v_ss)). But the Arabic warning v_msg still printed v_shortfall = the
-- shortfall at the FIRST/shallowest dip below zero. When the deficit DEEPENS later in the horizon the
-- user saw a small predicted shortage (v_shortfall) next to a much larger order qty (sized off v_maxdef)
-- — an internally inconsistent message.
--
-- THE FIX (one line, cosmetic): in the v_msg format(...) call, replace trim_scale(v_shortfall) with
-- trim_scale(greatest(v_shortfall, v_maxdef)) so the displayed predicted shortage matches what is being
-- ordered. v_maxdef >= v_shortfall always, so greatest(v_shortfall, v_maxdef) = v_maxdef whenever a
-- shortage exists; for a single-period dip (v_maxdef == v_shortfall) the message is byte-identical, so
-- this is a NO-OP for single-period shortage scenarios. The 'shortfall' JSON field is LEFT as
-- v_shortfall (still the first crossing, paired with first_shortage_period) — minimal, safe change.
-- recommend_qty, all JSON outputs, SECURITY DEFINER + set search_path='' are untouched.
--
-- Re-emitted BYTE-IDENTICAL to migration 0055 EXCEPT the single trim_scale(v_shortfall) -> 
-- trim_scale(greatest(v_shortfall, v_maxdef)) change in the v_msg format() call.
