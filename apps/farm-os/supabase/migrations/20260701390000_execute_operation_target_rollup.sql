-- Farm OS — execute-operation target rollup.
--
-- fn_execute_operation now supports multi-material execution (20260701230000). This migration
-- re-emits that current five-argument function and changes only the location write: planned
-- operations resolve farm/sector/hawsha/line/palm targets into the full ancestor chain so every
-- 360 page, field dashboard, and report sees the work connected to its parent structure.
-- Palm targets are also linked through event_assets for the existing palm activity readers.
...
begin
  ...
  -- the parent 360 pages and field dashboards. NULL target_type preserves the legacy sector-only
  -- write because old operations did not carry enough metadata to prove an ancestor chain.
  case
