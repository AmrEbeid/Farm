-- Farm OS — engine masked-shortage fix: count IN_PROGRESS (and approved) operation demand.
--
-- BUG (empirically reproduced; pinned by tests/102): the demand filter was `po.status in
-- ('planned','reserved','ready')`. But an operation whose work has STARTED ('in_progress') has NOT yet
-- issued its material — fn_execute_operation issues stock only at the execute→'done' flip, and EXE-STATUS
-- (0057) lists planned/reserved/ready/in_progress as "proceed normally". So in_progress carries real,
-- un-issued demand, yet the engine DROPPED it → reported shortage=false / recommend_qty=0 when stock was
-- actually short (the engine's cardinal sin, SPEC-0001 #1). `approved` was likewise excluded though the
-- app treats it as a live op (LIVE_OP in dashboard/owner).
--
-- FIX: align the demand + origin filters to the app's LIVE_OP set
-- ('planned','approved','reserved','ready','in_progress') — every non-terminal, non-blocked op with
-- un-issued material. Excluded (correctly): done (issued), abandoned/skipped (cancelled), blocked (paused).
-- Re-emitted VERBATIM from the 0094 definition; the ONLY change is the two status-IN lists (origin query
-- and demand query). Errs SAFE: adding a status can only ADD demand → raise a projected shortage / order,
-- never hide one (same safe direction as the reserved op already counted). Independent review required
-- (engine surface); pinned by the #239 oracle harness + the new tests/102.
