-- Farm OS MVP-0 — #198 (ENGINE-NULLDATE-1): a null-dated planned op must NOT silently drop its demand.
--
-- THE BUG (MED-latent, defence-in-depth). plan_operations.planned_at is NULLABLE. In fn_stock_coverage
-- (latest: migration 0045, the partial-receipts re-emit) the DEMAND (v_issues) query buckets each op by
--   greatest(floor((po.planned_at - v_period_start) / 7.0)::int + 1, 1)
-- and filters rows by the same expression <= p_horizon_weeks. When planned_at IS NULL both expressions
-- evaluate to NULL: the bucket is NULL and the `<= p_horizon_weeks` predicate is NULL (≠ true), so the
-- row is EXCLUDED. v_period_start := min(po.planned_at) also ignores NULLs. Net effect: a real material
-- requirement on an UNDATED op is invisible to the projection → it can mask a genuine shortage
-- (shortage=false / recommend_qty=0 when a purchase is actually needed). The app currently always sets
-- planned_at, so this is latent — but the column permits NULL and nothing in the engine guards against it.
--
-- THE FIX (conservative — only ever ADDS demand, never masks). Treat a null-dated op's demand as
-- IMMEDIATE (period 1): replace `po.planned_at` with `coalesce(po.planned_at, v_period_start)` in BOTH
-- the bucket expression and the `<= p_horizon_weeks` WHERE filter of the DEMAND (v_issues) query ONLY.
-- A null date then resolves to v_period_start → (date - date)/7 = 0 → bucket greatest(0+1,1)=1, and the
-- filter 1 <= p_horizon_weeks is true, so the row is INCLUDED in period 1. When v_period_start itself is
-- NULL it has already defaulted to current_date above, and coalesce(null, current_date)=current_date →
-- bucket 1 — handled. For ANY op with a non-null planned_at the coalesce is a no-op, so the projection
-- is byte-identical to migration 0045 (the existing engine tests — 04/06/16/35/40/45 + the seed, all of
-- which use dated ops — are unaffected). Nothing else changes: the receipts query, the #197 needed_by
-- guard, the #184 recommendation fix, and the partial-receipts `qty - received_qty` projection are all
-- byte-identical to 0045.
--
-- ADR-0006 conventions: forward-only migration; create-or-replace function; SECURITY DEFINER +
-- set search_path = ''; fully schema-qualified. The body below is re-emitted from its authoritative
-- latest definition (migration 0045) with ONLY the two coalesce() edits documented inline.
