-- Farm OS MVP-0 — #280 F4: size the purchase recommendation off the DEEPEST projected deficit, not
-- just the first dip below zero.
--
-- THE BUG (HIGH — under-orders a real shortage). fn_stock_coverage (latest: migration 0047) captures the
-- shortfall ONLY at the first period the projected available balance (PAB) crosses below zero:
--   if v_first is null and v_pab[i+1] < 0 then v_first := i; v_shortfall := -v_pab[i+1]; end if;
-- and sizes the recommendation off it: v_raw := greatest(0, v_shortfall + v_ss). When the deficit
-- DEEPENS later in the horizon (e.g. PAB = -10 at week 2 but -1010 at week 5 as more demand lands), the
-- recommendation covers only the first, shallow dip (~10) and leaves the farm ~1000 short at the worst
-- point — a silent under-order on exactly the shortages the engine exists to prevent.
--
-- THE FIX (conservative — only ever RAISES recommend_qty). Track v_maxdef = the worst deficit across the
-- whole horizon and size off greatest(0, v_maxdef + v_ss). Since the first crossing is one of the periods
-- scanned, v_maxdef >= v_shortfall always, so recommend_qty can only increase or stay equal — it can
-- never mask a shortage (safe direction). The 'shortfall'/first_shortage_period outputs are unchanged
-- (still the first crossing); only the recommendation sizing changes. The warning-only case (PAB below
-- safety stock but never below 0) is byte-identical: v_maxdef stays 0, so v_raw = v_ss as before.
--
-- Re-emitted VERBATIM from migration 0047 with ONLY: (1) the v_maxdef declaration, (2) the loop capture
-- of the max deficit, (3) the v_raw sizing line. SECURITY DEFINER + set search_path='' preserved.
