...
-- (Dr نقدية المبيعات / Cr ذمم مدينة) and payment_status is DERIVED from Σ(collections), never stored.
--
...
-- EXECUTE-locked, RLS + FORCE RLS + audit, mirroring the expenses/custody pattern. Non-authoritative until
-- the Owner applies (migrate-first) + merges.
begin;
...
-- Re-emitted from the latest source (S-3 migration 20260701450000, which added 'cost_center'). Adds
-- 'buyer' + 'sale_collection' to the finance.read gate; 'sale' is already gated (budget.write) below.
...
begin
  ...
  -- Derive payment_status from Σ(collections) (never store a running balance).
