...
-- 5) farm_event — performed_by/assigned_to person → people. CRITICAL: farm_event is PARTITIONED and its
-- tenant_all policy was created PER-PARTITION (migrations 0025/0035 loop over the parent + every
-- partition), because `authenticated` holds direct DML on the partition children — so a re-emit on the
-- parent ALONE leaves the children's stale policy in force and a `POST /rest/v1/farm_event_2025_07`
-- bypasses the new check. Mirror the established loop over all four relations. Columns are qualified with
-- %I so the EXISTS correlation `pe.org_id = <rel>.org_id` binds to the OUTER row (an unqualified org_id
-- inside the subquery would resolve to pe.org_id and pass vacuously).
do $$
declare t text;
begin
