...
export default async function AccountingPage() {
  ...
  const [trialRes, entriesRes, linesRes, accountsRes] = await Promise.all([
    ...
    sb
      ...
      .limit(80),
    sb.from("accounts").select("id, code, name_ar, account_type, normal_balance").order("code"),
  ]);
  ...
  const netByCode = (code: string) => Number(trialBalance.find((row) => row.code === code)?.net ?? 0);
