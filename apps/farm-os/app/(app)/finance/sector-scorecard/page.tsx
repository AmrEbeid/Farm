...
export default async function SectorScorecardPage() {
  ...
  for (const s of sales) {
    ...
    if (s.cost_center_id) revenueByCenter.set(s.cost_center_id, (revenueByCenter.get(s.cost_center_id) ?? 0) + v);
  ...
  const sectors: CenterPerf[] = rollup
    .filter((r) => r.active && !r.is_system && (r.area_feddan ?? 0) > 0 && !parentIds.has(r.cost_center_id))
    .map((r) => ({
      id: r.cost_center_id,
      name: r.name_ar,
  ...
  const unallocExpense = Number(rollup.find((r) => r.code === "CC-UNALLOC")?.net ?? 0);
