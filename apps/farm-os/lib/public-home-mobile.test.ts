...
describe("public homepage mobile layout", () => {
  it("collapses desktop navigation into a compact phone header", () => {
    ...
    expect(mobile).toContain(".site__nav { display: none; }");
  ...
  it("uses a compact phone hero and statistics strip", () => {
    expect(css).toContain(".site__hero-inner { padding: 2.25rem 1rem 3.25rem; }");
    expect(css).toContain(".site__title { max-width: none; font-size: 1.75rem;");
    expect(css).toContain(".site__stats { margin-block-start: -1.5rem;");
    expect(css.indexOf(".site__stat { border-radius: 8px;")).toBeGreaterThan(
