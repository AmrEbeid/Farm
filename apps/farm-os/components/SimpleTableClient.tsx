 ...
 */
export function SimpleTableClient({
  columns,
  rows,
  caption,
  ariaLabel,
  ...
  renderedCells?: Record<string, Record<string, ReactNode>>;
}) {
  ...
  return (
    <DataTable<SimpleRow>
      ...
      columns={columns.map((c, i) => ({
        ...
        cell: (row) => {
          const cell = renderedCells?.[row.id]?.[c.id] ?? (c.render ? c.render(row) : renderCell(c, row));
          return i === 0 && row.href ? (
            <Link
              href={row.href}
