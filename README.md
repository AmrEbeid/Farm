...
- **B — Farm OS app** (`apps/farm-os`): **MVP-0 DEPLOYED + LIVE** — Next.js + Supabase, the full stock-coverage wedge loop end-to-end. Auth is **email + password** (Supabase `signInWithPassword`). Independent security review done; **287 pgTAP + Playwright e2e** green. Live at **farm-ui-one.vercel.app** (+ `ebeidfarm.business`) on a dedicated cloud Supabase project (prod DB at **migration 0031** — repo `main` is at `0033`; `0032`/`0033` are verified on `main` but Owner-gated, not yet pushed to prod), running on synthetic seed data. Recharts is code-split via the `@amrebeid/ui/charts` subpath, so charts load only on the **2** chart routes (inventory coverage + planned-vs-actual report). Spec/plan in [`docs/superpowers`](docs/superpowers); deploy in [`docs/DEPLOY-RUNBOOK.md`](docs/DEPLOY-RUNBOOK.md) / status in [`docs/DEPLOY-STATUS.md`](docs/DEPLOY-STATUS.md).
