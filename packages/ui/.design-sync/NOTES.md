# design-sync NOTES — @amrebeid/ui

## General learnings
- [GENERAL] Source repo (no `node_modules/@amrebeid/ui`) → converter run with `--entry dist/index.js`; build the lib (`npm run build`) before each converter run so `dist/` is current.
