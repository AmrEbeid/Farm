...
| **FEAT-023** | Accounting P&L + sales (revenue/A-R) | 🧪 (draft PR #368 superseded; = Slice A of the roadmap, not yet built) | (`/accounting` route shipped via FEAT-030; P&L/sales not yet) | `fn_save_sale`, `fn_accounting_pnl_summary` (draft, superseded) | `lib/pnl.ts` (draft) | `88_accounting_pnl` (draft) | SPEC-0004 / `ROADMAP-accounting-custody-2026-07-01` |
...
| **FEAT-028** | Custody ledger & payment requests | ✅ (+ settlement/reports via FEAT-030) | `/custody`, `/custody/request/[requestId]`, `/finance/custody-reports` | `20260629150000`/`20260629150100`; `fn_save_custody_account`, `fn_record_custody_movement`, `fn_transfer_custody`, `fn_set_expense_payment_status`, `fn_create_payment_request`, `fn_add_expense_to_request`, lifecycle RPCs, `fn_payment_request_totals`; **settlement `20260701220000`: `fn_record_payment_request_funding`, `fn_confirm_request_expense_paid`, `fn_close_payment_request`**; **reports `20260701490000`: `fn_custody_ledger_report`, `fn_custody_cash_expense_report`, `fn_unpaid_obligations_report`, `fn_owner_funding_report`** | `CustodyForms.tsx`, `RequestLifecycle.tsx`, `app/(app)/finance/custody-reports/page.tsx`, `lib/request-lifecycle.ts` | `102_custody_payment`, `103_payment_request`, `112_accounting_cash_custody_settlement`, `119_custody_transfer`, `120_custody_reports`, `request-lifecycle.test.ts` | SPEC-0018 |
...
| **FEAT-030** | Accounting kernel — cash-method double-entry GL + custody settlement | ✅ (P&L/balance-sheet/sales ⬜ = roadmap Slice A; real-data reconcile Stage M) | `/accounting` (owner/accountant) | `20260701220000` (PR #568); `accounts`/`journal_entries`/`journal_lines`/`payment_request_fundings`; `fn_accounting_trial_balance`, `fn_post_two_line_journal`, `fn_ensure_account`, `fn_account_for_expense_kind` (internal); `journal_lines_balance_guard` | `app/(app)/accounting/page.tsx`, `CustodyForms.tsx` (settlement) | `112_accounting_cash_custody_settlement` | SPEC-0004 / `ROADMAP-accounting-custody-2026-07-01` |
