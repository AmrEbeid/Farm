...
| **TBL-048** | `journal_entries` | Double-entry journal header | id, org_id, entry_date, source_type, source_id, source_sequence, status∈{posted,reversed}, reversal_of? | organization, journal_entries(self) | UNIQUE(org_id,source_type,source_id,source_sequence); current posted source returns idempotently, while corrected re-posts after reversal advance the sequence (BR-117); reversals via `fn_reverse_journal_entry` (BR-132); reads `finance.read`; RPC-only; audited | FEAT-030 |
...
| **TBL-055** | `accounting_periods` | Closed/locked accounting periods (period lock, `20260701550000`) | id, org_id, period_start, period_end, status∈{open,locked}, note?, locked_by/at, reopened_by/at | organization | reads `finance.read`; close via `fn_close_accounting_period` (owner/accountant), reopen via `fn_reopen_accounting_period` (owner-only); a `locked` range blocks any NEW posting dated inside it (`fn_post_two_line_journal`); audited (entity `accounting_period`, gated in `audit_read`) | FEAT-030 |
