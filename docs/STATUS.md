...
*Updated: 2026-07-26 (accounting reconciliation stack migrated and merged; production deployment verified). Owner: Amr Ebeid.*

**Rule:** update this file whenever repo/prod state changes materially; keep it under ~100 lines. If this file and any other doc disagree, this file wins — then fix the other doc.

**2026-07-26 — accounting reconciliation provenance schema is live and merged.** PR #902 merged to `main` at
`d1c175e1`; PR #910 merged at `b4ab8ecf` after all three production migrations were applied. Post-merge application,
UI-library, secret-scan, release, and Vercel production checks pass. The full database harness is
...
No reconciliation evidence or financial row has been written. Slice 1A is complete; Slice 1B has not started.
...
| 7 Accounting | ✅ ~99% | GL kernel + custody/payment requests live (#568/#468); custody transfer/reports, revenue/A-R backend/reports, COA, cost centers, owner insights, offshoot valuation, close/period lock, trusted BS/IS/TB/budget-vs-actual, report catalog, proof packet, PDF/CSV coverage live. **Reversal RPC + audit_read pin + custody floor/journal-completeness shipped (#791/#792/#793); accounting-kernel correctness pass applied (#871, `20260708100000`): revenue posts on sale date, reversed-sale collection blocked, trial balance posted-only.** **Historical import/reconciliation DONE** (real 7-yr GL sheet-exact, `20260707115445`/`130001`/`131822` + reclass `20260708090000`). Remaining: Excel dual-run; deeper reversal semantics (reset source status / unwind collections / scorecards read posted-GL — product decision, latent). |
...
| M Real-data migration | ◐ Finance DONE / registry pending | **Finance history LIVE**: real 7-yr GL reconciled sheet-exact + opening balance + reclass, all applied to prod (2026-07-07/08). Remaining: **palm-registry import (#239, prod palms still synthetic)** + Stage 0 security (#362) + COA owner sign-off. |
