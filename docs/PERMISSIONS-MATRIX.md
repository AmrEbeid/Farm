...
| `/finance/dashboard` | `requireRole(["owner","accountant","farm_manager"])`; accountant branches before the legacy reader to accountant-only `fn_accountant_home_snapshot`, which rechecks active-org accountant membership; owner/farm-manager keep the existing role-aware snapshot | owner, accountant, farm_manager; accountant role landing |
