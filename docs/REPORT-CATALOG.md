...
| **RPT-41** | `/finance/dashboard` | Finance operating dashboard | approved budget, committed + actual, available budget, displayed expense/PR samples, exact unpaid known/unknown totals, custody/payment/journal queues when permitted | `BudgetDoughnut`, `VarianceChart` | Bounded exports only when complete; print-ready | RPC-078 atomic snapshot (including RPC-066/RPC-070 private summaries) | owner, accountant, farm_manager; unverified budgets withheld; drawings/accounting sections owner/accountant only |
