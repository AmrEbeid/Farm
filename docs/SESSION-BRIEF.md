...
4. **DS-rebuild batch** (`@amrebeid/ui`): `Field.required` **DONE (#627)** — completes F7's LaborLogForm (aria-hidden `*` marker + native `required`; danger-red `.fos-field__req`; changeset + dist committed). **KpiCard delta WCAG 1.4.1 still DEFERRED — needs an Owner design decision**, now with proof: a blanket ▲/▼ glyph turns the *overloaded* `deltaDirection` valence prop into a false **direction** claim at existing consumers — `plans/dashboard/page.tsx:375-379` (cost-variance would show ▼ under a positive/over-budget value) and `dashboard/owner/page.tsx:225-230` (▲ on a static overdue-PO count). Fix = EITHER audit+correct the ~4 mis-set consumers, OR add a `deltaTone="good"|"bad"|"none"` API and migrate all ~24. PageHeader/Stat consolidation also still pending.
