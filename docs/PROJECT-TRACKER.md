...
- [~] **Independent security review of the MVP-0 build** — DONE 2026-06-23 (3 adversarial subagents: RLS / grants / engine). Findings fixed on branch `fix/mvp0-security-remediation` (migration `0010` + test `05`): GRANT-C1 unauthenticated `anon` had full DML+EXECUTE incl. the SECURITY DEFINER engine (CRITICAL); RLS-H1 child tables didn't validate parent org (cross-tenant write, HIGH); ENGINE-C1 expiry double-counted (CRITICAL); ENGINE-H1 phantom purchase rec (HIGH); ENGINE-H2 horizon clamp + ENGINE-SS `first_warning_period`; plus hardening HIGH-1 (`organization`/`organization_member` client writes revoked — escalation lockdown) and ENGINE-M1 (forward-looking stock-out date). **49/49 pgTAP green** (36 existing preserved + 13 new) verified on a local PG (Docker stack was down). Remaining: Owner merge sign-off + Playwright e2e on the Docker/PostgREST stack. Deferred (needs the Docker stack to verify safely): FORCE RLS, `bin.reserved` ledger reconciliation. — owner: Amr
