...
| 7 | Accounting (expenses/sales/vouchers) | Execution | **High** | **Cash-method custody ledger in release (PR #568); full P&L still gated** | Branch `feat/accounting-custody-standalone` adds source-linked cash ledger + request settlement for custody/payment requests. Validation green (local pgTAP 904/904, app 251/251, lint/build, PR checks + CodeRabbit). Prod migration `20260701220000 accounting_cash_custody_settlement` is applied/probed migrate-first; merge/deploy verification remains pending. Older #368 synthetic P&L remains behind real Excel reconciliation + Stage-M privacy review. |
