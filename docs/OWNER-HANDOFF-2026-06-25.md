...
| **`@amrebeid/ui` 1.2.0 release** | ✅ **DONE (2026-06-25).** The 4 queued changesets + the **Storybook 10 toolchain upgrade** (PR #154) shipped: the changesets release flow published **`1.2.0` to npm** and pushed tag `@amrebeid/ui@1.2.0` (release PR #162 merged). `packages/ui/package.json` now `1.2.0`. | npm, git tag `@amrebeid/ui@1.2.0`, release run on PR #162 | **None** — released. If the app should consume the new lib, rebuild + commit `packages/ui/dist/` (app currently consumes the in-repo prebuilt dist). |
| **Key rotation** | Supabase DB password + `service_role` key were shared out-of-band; the **demo login password is committed** (`lib/seed-auth.ts` + `app/login/page.tsx`, client-bundled — corrected 2026-06-26, was wrongly logged "not committed"). **Deferred to project end** per current plan. | `docs/DEPLOY-STATUS.md` "Security follow-ups" (#1–4) | **Owner, before any non-demo/real use:** reset DB password, roll `service_role` (update Vercel `SUPABASE_SERVICE_ROLE_KEY` + redeploy), **de-hardcode** + rotate/delete demo logins. *(A gitleaks CI gate now blocks NEW committed secrets.)* |
